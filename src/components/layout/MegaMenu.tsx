...
import { ArrowRightIcon, MagnifyingGlassIcon, Squares2X2Icon } from '@heroicons/react/20/solid'
...
/* ─────────────────────── 说明：分类切换无动画 ───────────────────────
   面板内所有内容切换（左侧导航 hover 切换分类、搜索过滤）一律即时渲染。
   曾用 AnimatePresence + 淡入淡出，但在 hover 驱动下会造成内容区闪烁，
   见内容主体处的注释与规范文档记录。 */
              ...
              {categories.map((category) => {
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      categoryButtonBase,
                      isActive ? categoryButtonActive : categoryButtonIdle
                    )}
                  >
          ...
          <div className={megaMain}>
            ...
            {quickTags && quickTags.length > 0 && (
              <div className="flex items-center gap-2 border-b border-neutral-100 py-3">
                <span className="shrink-0 text-xs text-neutral-500">快速直达：</span>
            ...
            {/* ── 内容主体 ──
                分类切换必须「即时」：左侧导航是 hover 切换，任何淡入淡出都会在
                鼠标扫过时造成内容区整块淡出→淡入的闪烁
                （2026-09-22 实测：AnimatePresence 每次切换产生 ~320ms 空白）。
                因此这里不用 AnimatePresence / 入场动画，直接渲染当前分类内容。 */}
                ...
                <div>
                  ...
                  {filteredItems.length > 0 && (
                    <section>
                      {activeCategory.featured &&
                        activeCategory.featured.length > 0 &&
                        !debouncedQuery && <SectionTitle text="全部产品" />}
                  ...
                  {filteredItems.length === 0 && debouncedQuery && (
                    <div className="flex flex-col items-center justify-center py-16 text-neutral-400">
                      ...
                      <p className="text-sm font-medium text-neutral-700">未找到相关产品</p>
                      <p className="mt-1 text-xs text-neutral-500">请尝试其他关键词搜索</p>
                    </div>
                  )}
                </div>
              )}
                  ...
                  <div className="space-y-0.5">
                    {footerActions.map((action) => {
                      ...
                      return (
                        ...
                        >
                          {Icon && (
                            <Icon aria-hidden="true" className="size-5 shrink-0 text-brand-500" />
                          )}
                          {action.name}
                          <ArrowRightIcon
                            aria-hidden="true"
...
const ProductLink = React.memo(function ProductLink({
  ...
  return (
    <Link href={item.href} onClick={onClick} className={productLink}>
      ...
      {/* 第二行：描述贴左对齐 */}
      {item.description && (
        <p className="mt-1 line-clamp-1 text-left text-xs text-neutral-500">{item.description}</p>
      )}
    </Link>
  )
})
