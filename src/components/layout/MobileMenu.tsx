    ...
    >
      {item.icon && (
        <span className="mt-0.5 flex size-5 flex-none items-center justify-center">
          <item.icon aria-hidden="true" className="size-4 shrink-0 text-brand-500" />
        </span>
      )}
      ...
      <div>
        ...
        <div className="flex items-center gap-2 border-b border-neutral-200 pb-2">
          ...
          <span className="flex min-w-0 items-center gap-2">
            {activeSection.icon && (
              <activeSection.icon aria-hidden="true" className="size-4 shrink-0 text-brand-500" />
            )}
        ...
        <div className="mt-3 flex items-stretch">
          ...
          <main className="ml-3 min-w-0 flex-1 border-l border-neutral-200 pl-3">
            {activeCategory ? (
              ...
              <p className="px-2 py-6 text-center text-xs text-neutral-400">该分区暂无内容</p>
            )}
          </main>
        </div>
      </div>
      ...
      <div className="mt-3 space-y-0.5 border-t border-neutral-200 pt-3">
        ...
        <Link
          ...
          className={`block w-full rounded-md py-2 pr-1 pl-2 text-sm font-medium transition-colors hover:bg-neutral-50 ${
            eccloudActive ? 'text-brand-600' : 'text-neutral-700 hover:text-brand-600'
          }`}
        >
          电商云
        </Link>
