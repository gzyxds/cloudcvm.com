...
/**
 * 产品特性展示组件 — 现代 SaaS 极简风格
 *
 * 设计参考 Linear / Vercel / Stripe：
 * - 顶部平台指标概览条（类比控制台 Dashboard 资源统计）
 * - 4 列卡片网格（带度量标签、操作入口）
 * - 无边框 + 微阴影层次 + 微圆角(rounded-md)
 * - 图标渐变底色；悬停上浮 + 阴影加深 + 品牌色顶边高亮
        ...
        <div className="mt-14 flex flex-wrap items-start justify-center gap-x-12 gap-y-6">
          {platformMetrics.map((metric) => (
            <div key={metric.label} className="flex flex-col items-center gap-1">
              <metric.icon className="h-5 w-5 text-[#0055ff]/60" />
            ...
            {productTraits.map((trait) => (
              <div
                ...
                className={clsx(
                  ...
                  'focus-within:shadow-[0_0_0_1px_rgba(0,85,255,0.15),0_8px_24px_rgba(15,23,42,0.06)] focus-within:before:via-[#0055ff]/40',
                )}
                tabIndex={0}
                role="article"
