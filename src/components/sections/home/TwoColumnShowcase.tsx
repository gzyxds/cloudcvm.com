...
export default function TwoColumnShowcase() {
  ...
  // 获取当前选中的项目数据
  const currentItem =
    currentCategory.items.find((i) => i.id === activeItem) || currentCategory.items[0]
  ...
  /**
   * Tab 悬停切换：延迟 100ms，避免鼠标快速划过时频繁切换
   * （卸载清理由 Hook 统一负责）
   */
          ...
          <div className="w-full lg:col-span-7">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="w-full"
              >
                <ImagePreview imageUrl={currentItem.imageUrl} title={currentItem.title} />
              </motion.div>
            </AnimatePresence>
          </div>
        </div>
