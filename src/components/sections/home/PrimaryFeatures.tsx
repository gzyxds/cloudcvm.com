...
function CardBody({ feature }: { feature: Feature }) {
  return (
    <>
      ...
      {/* Description */}
      <p className="mt-1.5 text-sm leading-[26px] tracking-wide text-neutral-500">
        {feature.description}
      </p>
      ...
      <div className="mt-3 flex-1 space-y-[6px]">
        {feature.features.map((item, i) => (
          <div key={i} className="flex items-start">
            <span className="mt-[5px] mr-2.5 inline-block flex-shrink-0">
              <CheckIcon className="h-3.5 w-3.5 text-brand-500" />
            </span>
            <span className="text-sm leading-relaxed">
              <span className="font-semibold tracking-tight text-neutral-900">{item.title}</span>
      ...
      {feature.customers && feature.customers.length > 0 && (
        <div className="mt-7">
          <h4 className="text-xs font-semibold tracking-widest text-neutral-900 uppercase">客户</h4>
          <div className="mt-3 flex flex-wrap">
            {feature.customers.map((cust, i) => (
              <a
                key={i}
                href={cust.href}
                className="mr-3 mb-1 w-fit rounded-lg border border-neutral-200 bg-white px-[21px] py-[3px] text-sm text-neutral-900 transition-colors hover:border-brand-500 hover:text-brand-500"
              >
                {cust.name}
              </a>
            ))}
      ...
      {feature.products.length > 0 && (
        <div className="mb-7">
          <h4 className="mb-3 text-xs font-semibold tracking-widest text-neutral-900 uppercase">
            相关产品
          </h4>
          <div className="flex flex-wrap gap-x-6 gap-y-1 text-neutral-500">
            {feature.products.map((prod, i) => (
              <a
                key={i}
                href={prod.href}
                className="flex items-center text-sm leading-6 transition-colors hover:text-brand-500"
              >
                {prod.name}
              </a>
            ))}
...
function TabBar({
  ...
  return (
    <div className="scrollbar-hide -mx-4 flex overflow-x-auto px-4 lg:mx-0 lg:px-0">
      <div className="inline-flex flex-nowrap rounded-lg bg-neutral-100 p-1 whitespace-nowrap">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => onTabChange(tab.id)}
            className={clsx(
              'flex-shrink-0 rounded-md px-4 py-2.5 text-sm font-medium tracking-wide transition-all duration-200 lg:px-5',
              activeTab === tab.id
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0" style={{ background: feature.gradient, opacity: 0.6 }} />
        <div className="relative z-10 flex h-full flex-col pt-9 pr-[16px] pl-[29px] text-white">
          ...
          <p className="mt-1.5 line-clamp-2 text-sm leading-relaxed tracking-wide text-white/75">
            {feature.description}
          </p>
        </div>
      </div>
    ...
    <div className="overflow-hidden rounded-[18px] border border-neutral-200 bg-white">
      ...
      <div className="relative flex items-center gap-3 overflow-hidden px-5 py-5">
        <div
          ...
          style={{ backgroundImage: `url(${feature.bgImage})` }}
        />
        <div className="absolute inset-0" style={{ background: feature.gradient, opacity: 0.65 }} />
        <feature.icon className="relative z-10 h-6 w-6 flex-shrink-0 text-white" />
      ...
      <Container className="relative z-10">
        ...
        <div className="flex items-end justify-between">
          ...
          <div className="hidden items-center text-sm font-medium tracking-wide text-neutral-900 sm:flex">
            <a
              href="/new"
              className="group mr-[48px] inline-flex items-center gap-1.5 leading-8 transition-colors hover:text-brand-500"
            >
              <span>全部方案</span>
            ...
            <a
              href="/contact"
              className="group inline-flex items-center gap-1.5 leading-8 transition-colors hover:text-brand-500"
            >
              <span>架构咨询</span>
              <ArrowRightIcon className="h-3.5 w-3.5 transition-transform group-hover:translate-x-1" />
            </a>
        ...
        {/* ================================================================ */}
        <div className="mt-8 hidden h-[547px] w-full gap-3 lg:flex">
          ...
          >
            ...
            <ArrowRightIcon className="absolute bottom-[20px] h-4 w-4 text-neutral-400 transition-colors group-hover:text-brand-500" />
          </a>
        </div>
        ...
        <div className="mt-6 space-y-4 lg:hidden">
          ...
          <a
            href="/new"
            className="flex w-full items-center justify-center gap-2 rounded-md border border-neutral-200 bg-neutral-50 py-4 text-sm font-semibold tracking-wide text-neutral-700 transition-colors hover:border-brand-300 hover:text-brand-500"
          >
            查看更多方案
