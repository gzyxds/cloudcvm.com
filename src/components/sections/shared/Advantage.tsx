...
export default function Advantage() {
  ...
  useEffect(() => {
    ...
    const buttons = Array.from(journey.querySelectorAll<HTMLButtonElement>('[data-advantage-jump]'))
    const panels = Array.from(journey.querySelectorAll<HTMLElement>('[data-advantage-panel]'))
    ...
    const routeD = (centerY: number): string => {
      ...
      const pointX = (y: number): number => {
        ...
        const progress = (curveRadius - distance) / (curveRadius - straightRadius)
    ...
    const renderRoute = (snapToActive = false): void => {
      ...
      const height = routeEl.clientHeight || Math.max(0, buttons.length * 80)
      routeEl.setAttribute('viewBox', `0 0 ${width} ${height}`)
      state.routeYs = buttons.map((button) => {
        const rect = button.getBoundingClientRect()
        return rect.top + rect.height / 2 - Math.max(routeRect.top, navRect.top + 55)
      })
    ...
    const animateRoute = (index: number): void => {
      ...
      const animate = (time: number): void => {
        ...
        state.routeCenterY = startCenterY + (targetCenterY - startCenterY) * eased
        routePathEl.setAttribute('d', routeD(state.routeCenterY))
        if (progress < 1) {
    ...
    >
      ...
      <Container className={styles.headingWrap}>
        <header className={styles.heading}>
          <h2>产品优势</h2>
          <p>
            优刻云云服务器 ECS是优刻云提供的性能卓越、稳定可靠、弹性扩展的IaaS（Infrastructure as a
            Service）级别云计算服务。选择云服务器 ECS，您可以轻松构建具备以下优势的计算资源。
          </p>
        </header>
      </Container>
      ...
      <Container className={styles.layout}>
        ...
        <aside className={styles.nav} aria-label="产品优势导航">
          ...
          <svg ref={routeRef} className={styles.route} aria-hidden="true" data-advantage-route>
            <path ref={routePathRef} className={styles.routePath} data-advantage-route-path />
          </svg>
            ...
            return (
              ...
              >
                ...
                <span className={styles.navText}>
                  ...
                  <strong className={styles.navStage}>{advantage.category}</strong>
                  <small className={styles.navTag}>{advantage.metric}</small>
                </span>
              </button>
            )
          })}
            ...
            return (
              ...
              >
                ...
                <div className={styles.panelTop}>
                  ...
                  <em className={styles.panelMeta}>核心指标 {advantage.metric}</em>
                </div>
                ...
                <div className={styles.panelGrid}>
                  ...
                  <div
                    ...
                    aria-hidden="true"
                  >
                    {/* 静态导出下 images.unoptimized，next/image 与原生 img 等价，
                                            此处保留原生 img 以复用 CSS Module 的样式钩子 */}
                    <img
                      src={shot}
                      alt=""
                ...
                <div className={styles.panelBottom}>
                  <div>
                    <small>核心能力</small>
                    {advantage.highlights.map((highlight, highlightIndex) => (
                      <span key={highlightIndex} className={styles.bottomPill}>
                        {highlight}
                      </span>
                    ))}
                  </div>
