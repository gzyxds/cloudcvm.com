...
function Tag({
  ...
  return (
    <div
      className={clsx(
        'rounded-md border px-3 py-1 text-[13px] leading-6 font-medium transition-colors sm:text-sm',
...
function LinkWithArrow({
  ...
  return (
    <a
      ...
      rel="noopener"
      className={clsx(
        'group flex items-center rounded-md text-[15px] font-medium whitespace-nowrap text-neutral-900 transition-colors outline-none hover:text-brand-500 focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 sm:text-base',
        className
      )}
    >
      <span>{children}</span>
      <ArrowRight
        className="ml-1.5 h-4 w-4 translate-x-0 transition-transform duration-200 group-hover:translate-x-1"
        aria-hidden="true"
      />
    </a>
  )
...
function DocLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a
      ...
      target="_blank"
      rel="noopener"
      className="group flex items-center rounded-md text-[15px] font-medium text-neutral-600 transition-colors outline-none hover:text-brand-500 focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 sm:text-base"
    >
      <span>{children}</span>
      <ArrowRight
...
/**
 * 服务卡片数据类型定义 - 企业级服务条目信息结构
 *
 * 命名为 ServiceProduct 而非 Product：与 `@/types/product` 中的「可购买商品」
 * 语义不同（此处是服务条目，无价格/购买链接），避免同名混淆。
 */
...
const serviceTabs: ServiceTab[] = [
  {
    ...
    products: [
      ...
      },
      {
        id: 'ddos',
        name: 'DDoS 攻击防护',
        description: '国内5大清洗中心，海外8大节点的Anycast任播网络，优质“近源防护”',
        tags: ['全球分布', '智能防护', '稳定可靠'],
        link: 'https://console.cloudcvm.com/cart/goodsList.html',
      },
      {
...
export default function ServiceTabs() {
  ...
  return (
    ...
    >
      {/* 装饰性元素 - 浮动圆点 */}
      <div className="absolute top-20 left-10 h-40 w-40 rounded-full bg-brand-400/[0.12] blur-3xl" />
      <div className="absolute right-20 bottom-20 h-56 w-56 rounded-full bg-brand-500/[0.10] blur-3xl" />
      <div className="absolute top-1/2 left-1/2 h-80 w-80 -translate-x-1/2 -translate-y-1/2 rounded-full bg-brand-300/[0.08] blur-3xl" />
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1800px] px-3 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 sm:mb-8">
          {/* 移动端 Tab - 参考 TwoColumnShowcase.tsx 设计：横向滚动 + 底部下划线指示器 */}
          <div className="scrollbar-hide -mx-3 overflow-x-auto border-b border-neutral-200 px-3 sm:hidden">
              ...
              {serviceTabs.map((tab, index) => {
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      'relative flex items-center justify-center gap-1.5 px-3 py-3 text-sm font-medium whitespace-nowrap transition-colors duration-200 outline-none select-none',
                      'min-h-[44px] flex-shrink-0',
                      'focus-visible:z-10 focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-inset',
              ...
              {serviceTabs.map((tab, index) => {
                ...
                return (
                  <button
                    ...
                    aria-controls={`tabpanel-${tab.id}`}
                    className={clsx(
                      'flex flex-1 items-center justify-center gap-2 rounded-md px-5 py-3 text-base font-medium whitespace-nowrap transition-all duration-200 outline-none select-none lg:px-7',
                      'min-h-[48px]',
                      'focus-visible:z-10 focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-inset',
                      isActive
        ...
        >
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="flex w-full flex-col gap-4 sm:gap-5 lg:flex-row"
            >
              {/* 左侧区域 */}
                  ...
                  <div className="relative z-10 flex h-full flex-col">
                    <div className="flex-1">
                      <div className="mb-4 flex flex-col gap-3">
                        <h3 className="text-xl font-semibold tracking-tight text-neutral-900 transition-colors group-hover:text-brand-500 sm:text-2xl">
                          {banner.title}
                        </h3>
                      ...
                      <p className="border-t border-neutral-200 pt-4 text-sm leading-relaxed text-neutral-600 sm:text-base">
                        {banner.description}
                      </p>
                      ...
                      <div className="mt-6 flex items-center gap-4">
                        ...
                        {banner.link && <DocLink href={banner.link}>了解详情</DocLink>}
                      </div>
                    </div>
                ...
                >
                  {secondaryBanner.link ? (
                    <a
                      ...
                      target="_blank"
                      rel="noopener"
                      className="relative z-10 flex h-full w-full flex-col justify-center rounded-lg p-5 outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-inset sm:p-6"
                    >
                      <div className="mb-2 flex items-center justify-between">
                        <div className="flex-1 pr-4 text-lg font-semibold tracking-tight text-neutral-900 transition-colors group-hover:text-brand-500 sm:text-xl">
                          {secondaryBanner.title}
                        </div>
                        <ArrowRight
                          className="h-5 w-5 text-neutral-400 transition-all group-hover:translate-x-0.5 group-hover:text-brand-500"
                          aria-hidden="true"
                        />
                      </div>
                      <div className="flex flex-wrap gap-2">
                    ...
                    <div className="flex w-full flex-col justify-center p-5 sm:p-6">
                      <div className="mb-2 text-lg font-semibold tracking-tight text-neutral-900 sm:text-xl">
                        {secondaryBanner.title}
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {secondaryBanner.tags.map((tag, i) => (
                      ...
                      <article className="group relative flex h-full cursor-pointer flex-col justify-start rounded-md border border-neutral-200 bg-white p-4 transition-all duration-200 focus-within:ring-2 focus-within:ring-brand-500 focus-within:ring-inset hover:border-brand-500/30 hover:shadow-md">
                        <div className="w-full">
                          <div className="mb-3 flex items-center justify-between">
                            ...
                            <ArrowRight
                              className="h-4 w-4 text-neutral-400 opacity-0 transition-all group-hover:translate-x-0.5 group-hover:text-brand-500 group-hover:opacity-100"
                              aria-hidden="true"
                            />
                          </div>

                          <p className="mb-3 line-clamp-2 min-h-[40px] text-sm leading-relaxed text-neutral-500 group-hover:text-neutral-700">
                            {product.description}
                          </p>
                          ...
                          <div className="mt-auto flex flex-wrap gap-1.5">
                            {product.tags.map((tag, tIdx) => (
                              <span
                                key={tIdx}
                                className="rounded-md border border-neutral-200 bg-neutral-50 px-2 py-0.5 text-xs text-neutral-500 transition-colors group-hover:border-brand-300 group-hover:bg-brand-50 group-hover:text-brand-500"
                              >
                                {tag}
                              </span>
                            ))}
