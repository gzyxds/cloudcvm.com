...
const solutionCards: SolutionCard[] = [
  {
    ...
    subtitle: '解决方案',
    description:
      '依托豆包大模型，提升全域数字营销能力。在 AI 赋能下，让用户洞察更精准，让销售效率更高，线索实现更高转化',
    icon: Monitor,
  ...
  },
  {
    ...
    subtitle: '解决方案',
    description:
      '提供算力、模型、平台、应用全栈大模型解决方案，在对公业务、零售业务、研发支持、办公助手等场景实现大模型升级',
    icon: Landmark,
      ...
      <Container>
        ...
        <div className="relative">
          ...
          <div
            ...
            className="flex snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth pb-4 [-ms-overflow-style:none] [scrollbar-width:none] lg:gap-6 [&::-webkit-scrollbar]:hidden"
          >
            {solutionCards.map((card) => (
              <div
                key={card.title}
                className="h-[400px] w-[85vw] flex-shrink-0 snap-start sm:h-[440px] sm:w-[calc((100%-1rem)/2)] md:w-[calc((100%-2rem)/3)] lg:h-[460px] lg:w-[calc((100%-4.5rem)/4)] xl:w-[calc((100%-6rem)/5)]"
              >
          ...
          {/* 左右箭头导航和分页器 */}
          <div className="mt-6 flex items-center justify-center gap-4 sm:mt-8 sm:gap-6 lg:mt-10">
            <button
              onClick={handlePrev}
            ...
            {/* 页码指示器 - 增大触控区域并优化动画 */}
            <div className="flex items-center gap-2 sm:gap-3" role="tablist" aria-label="轮播导航">
              {Array.from({ length: maxIndex + 1 }).map((_, index) => (
                <button
                  key={index}
                  onClick={() => scrollTo(index)}
                  className="group relative flex h-6 w-6 items-center justify-center focus:outline-none sm:h-5 sm:w-5"
